...
const { hkdfExtract, hkdfExpandLabel, derivePacketKeys } = require('./crypto');
...
function derivePSK(hashAlgo, ticketKey, encryptedTicket) {
  ...
  try {
    ...
    // YENİ: Doğru GCM parçalama sırası
    const iv  = encryptedTicket.subarray(0, 12);
    // Tag her zaman dizinin en sonundaki 16 byte'tır
    const tag = encryptedTicket.subarray(encryptedTicket.length - 16);
    // Ciphertext, IV ile Tag arasında kalan tüm veridir
    const ct  = encryptedTicket.subarray(12, encryptedTicket.length - 16);

    // YENİ: Anahtar uzunluğuna göre doğru AES algoritmasını seç
    const algo = ticketKey.length === 32 ? 'aes-256-gcm' : 'aes-128-gcm';
    const decipher = crypto.createDecipheriv(algo, ticketKey, iv);
